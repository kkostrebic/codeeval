...
# You are given a piece of text. Your job is to write a program that sets the case of text characters according to the
# following rules:
#
...
#
# INPUT SAMPLE:
# The first argument will be a path to a filename containing sentences, one per line. You can assume that all characters
...
File.open(ARGV[0]).each_line do |line|
  ...
  line.split('').each do |char|
    if /[A-Za-z]/.match(char).nil?
      result << char
    else
      result << ((index % 2 == 0) ? char.upcase : char.downcase)
      index += 1
    end
  end
