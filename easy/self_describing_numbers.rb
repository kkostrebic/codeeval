...
# 1
#
# For the curious, here's how 2020 is a self-describing number: Position '0' has value 2 and there is two 0 in the
# number. Position '1' has value 0 because there are not 1's in the number. Position '2' has value 2 and there is two 2.
...
File.foreach(ARGV[0]) do |line|
  ...
  numbers.each_with_index do |number, index|
    number = number.to_i
    if ((fill_len + number > len) || (numbers.select { |item| item == index.to_s }.count != number))
      result = 0
      break
    end
